...
`launch()` will automatically download the application files into a folder in the current working directory. If the branch is storing an app-package, `launch()` loads the package and then launches the application:

```{r , eval=FALSE}
launch(app = "09b_inst-bslib")
```
...
```{r , eval=FALSE}
is_r_package("09b_inst-bslib", verbose = TRUE)
```
