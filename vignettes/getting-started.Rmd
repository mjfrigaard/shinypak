...
```{r launch_app, echo=FALSE, out.width='80%', fig.align='center'}
knitr::include_graphics("../man/figures/launch_app.gif")
```

`launch_app()` will automatically download the application files into a folder in the current working directory. If the branch is storing an app-package, `launch_app()` loads the package and then launches the application:

```{r , eval=FALSE}
launch_app(app = "09b_inst-bslib")
```

```{r launch_app_pkg, echo=FALSE, out.width='80%', fig.align='center'}
...
```{r , eval=FALSE}
check_r_pkg("09b_inst-bslib", verbose = TRUE)
```
...
`launch_app()` calls the `get_app()` function, which will only download the specified branch into the current working directory. 

```{r , eval=FALSE}
get_app(app = "14_tests-system")
```

```{r get_app, echo=FALSE, out.width='80%', fig.align='center'}
...
```{r get_app_open, echo=FALSE, out.width='80%', fig.align='center'}
